function [feature_sogg_60pc,feature_sogg_40pc,time_extraction_feature2_1] = feature_freq_time_domain2_1r(data,labels)
tic;   
fc=250; %Hz
vv=round(0.6*size(data,1));
rr=size(data,1);
...
data2_std(1:round(0.4*rr),1:32)=std(data(vv+1:rr,:,:),0,3);
data_2var(1:round(0.4*rr),1:32)=var(data(vv+1:rr,:,:),0,3);
...
picco3(1:round(0.4*rr),:)=picco2(vv+1:rr,:);
%RMS
for o=1:vv
    for j=1:32
...
rms_data3(1:round(0.4*rr),:)=rms_data2(vv+1:rr,:);
...
peaktopeak3(1:round(0.4*rr),:)=peaktopeak2(vv+1:rr,:);
...
% PSD3 = zeros(round(0.2*rr), 32, psd_size2);
 PSD2=zeros(round(0.4*rr),32*num_bands);
for s=vv+1:rr
    for n=1:32
...
for trial=vv+1:rr
 ...
 signal_STFT3(1:round(0.4*rr),:)=signal_STFT2(vv+1:rr,:);
...
wavelet_energy3(1:round(0.4*rr),:)=wavelet_energy2(vv+1:rr,:);
...
features3= zeros(round(0.4*rr), size(filter_bands, 1) * size(csp_filters{1}, 2));
for i = 1:size(filter_bands, 1)
    for trial = vv+1:rr
        csp_projection = squeeze(data(trial, :, :))' * csp_filters{i};
...
features4(1:round(0.4*rr),:)=features3(vv+1:rr,:);
%calcolo ERD/ERS
event=1000/2;
eeg_pre=event-200;
eeg_post=event+300;
...
feature_sogg_60pc=zeros(vv,18);
feature_sogg_60pc(1:vv,1:32,1)=data1_m(1:vv,1:32);
feature_sogg_60pc(1:vv,33:64,1)=data1_std(1:vv,1:32);
feature_sogg_60pc(1:vv,65:96,1)=picco(1:vv,1:32);
feature_sogg_60pc(1:vv,97:128,1)=rms_data(1:vv,1:32);
feature_sogg_60pc(1:vv,129:160,1)=peaktopeak(1:vv,1:32);
feature_sogg_60pc(1:vv,161:192,1)=signal_STFT(1:vv,1:32);
feature_sogg_60pc(1:vv,193:224,1)=k(1:vv,1:32);
feature_sogg_60pc(1:vv,225:288,1)=PSD(1:vv,1:64);
feature_sogg_60pc(1:vv,289:320,1)=wavelet_energy(1:vv,1:32);
feature_sogg_60pc(1:vv,321:352,1)=skew_data(1:vv,1:32);
feature_sogg_60pc(1:vv,353:384,1)=data_var(1:vv,1:32);
feature_sogg_60pc(1:vv,385:416,1)=erd_ers_per_trial(1:vv,1:32);
feature_sogg_60pc(1:vv,417:434,1)=features(1:vv,1:18);
feature_sogg_60pc(1:vv,435:466,1)=features_encode(1:vv,1:32);
feature_sogg_60pc(1:vv,467:466+size(features_chc,2),1)=features_chc(1:vv,1:size(features_chc,2));




feature_sogg_40pc(1:round(0.4*rr),1:32,1)=data2_m(1:round(0.4*rr),1:32);
feature_sogg_40pc(1:round(0.4*rr),33:64,1)=data2_std(1:round(0.4*rr),1:32);
feature_sogg_40pc(1:round(0.4*rr),65:96,1)=picco3(1:round(0.4*rr),1:32);
feature_sogg_40pc(1:round(0.4*rr),97:128,1)=rms_data3(1:round(0.4*rr),1:32);
feature_sogg_40pc(1:round(0.4*rr),129:160,1)=peaktopeak3(1:round(0.4*rr),1:32);
feature_sogg_40pc(1:round(0.4*rr),161:192,1)=signal_STFT3(1:round(0.4*rr),1:32);
feature_sogg_40pc(1:round(0.4*rr),193:224,1)=k3(1:round(0.4*rr),1:32);
feature_sogg_40pc(1:round(0.4*rr),225:288,1)=PSD2(1:round(0.4*rr),1:64);
feature_sogg_40pc(1:round(0.4*rr),289:320,1)=wavelet_energy3(1:round(0.4*rr),1:32);
feature_sogg_40pc(1:round(0.4*rr),321:352,1)=skew_data2(1:round(0.4*rr),1:32);
feature_sogg_40pc(1:round(0.4*rr),353:384,1)=data_2var(1:round(0.4*rr),1:32);
feature_sogg_40pc(1:round(0.4*rr),385:416,1)=erd_ers_per_trial2(1:round(0.4*rr),1:32);
feature_sogg_40pc(1:round(0.4*rr),417:434,1)=features4(1:round(0.4*rr),1:18);
feature_sogg_40pc(1:round(0.4*rr),435:466,1)=features_encode(vv+1:rr,1:32);
feature_sogg_40pc(1:round(0.4*rr),467:466+size(features_chc,2),1)=features_chc(vv+1:rr,1:size(features_chc,2));

time_extraction_feature2_1=toc;
end
