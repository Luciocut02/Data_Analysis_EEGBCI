function [feature_sogg_60cs2_1,feature_sogg_40cs2_1,time_extraction_featurecs2_1] = feature_freq_time_domaincs2_1(matrix_train_60_40cs,matrix_test_60_40cs,labels_train_60_40cs)
tic;   
fc=250; %Hz
vv=size(matrix_train_60_40cs,1);
rr=size(matrix_test_60_40cs,1);

%extraction statistic feature
data1=matrix_train_60_40cs(1:vv,:,:).^2;
data1_sum=sum(data1,3);
data1_m=mean(data1_sum,3);
data1_std(1:vv,1:32)=std(matrix_train_60_40cs(1:vv,:,:),0,3);
data_var(1:vv,1:32)=var(matrix_train_60_40cs(1:vv,:,:),0,3);

data2=matrix_test_60_40cs(1:rr,:,:).^2;
data2_sum=sum(data2,3);
data2_m=mean(data2_sum,3);
data2_std(1:rr,1:32)=std(matrix_test_60_40cs(1:rr,:,:),0,3);
data_2var(1:rr,1:32)=var(matrix_test_60_40cs(1:rr,:,:),0,3);

%feature extraction-->time domain
%Peak latency
for s=1:vv
    for n=1:32
         [f,r]=max(matrix_train_60_40cs(s,n,:),[],3);
          picco(s,n)=r;
...
for s=1:rr
    for n=1:32
         [f,r]=max(matrix_test_60_40cs(s,n,:),[],3);
          picco2(s,n)=r;
    end
end
picco2=picco2./fc;
...
for o=1:vv
    for j=1:32
        rms_data(o,j)=rms(matrix_train_60_40cs(o,j,:),"all");
    end
end
% rms_data=mean(rms_data,2);
for o=1:rr
    for j=1:32
        rms_data2(o,j)=rms(matrix_test_60_40cs(o,j,:),"all");
    end
end
...
for o=1:vv
    for j=1:32
        peaktopeak(o,j)=max(squeeze(matrix_train_60_40cs(o,j,:)))-min(squeeze(matrix_train_60_40cs(o,j,:)));
    end
end
...
for o=1:rr
    for j=1:32
        peaktopeak2(o,j)=max(squeeze(matrix_test_60_40cs(o,j,:)))-min(squeeze(matrix_test_60_40cs(o,j,:)));
    end
end
...
for s = 1:vv
    ...
    for n = 1:32
        ...
        [pxx, f] = pwelch(squeeze(matrix_train_60_40cs(s, n, :)), [], [], [], fc);
...
for s=1:rr
    for n=1:32
        [pxx2, f2]=pwelch(squeeze(matrix_test_60_40cs(s,n,:)),[],[],[],fc);
        for b = 1:num_bands
            % Estrazione degli indici della frequenza per la banda corrente
...
for trial=1:vv
    for chn=1:32
        signal_STFT(trial,chn,:,:)=abs(stft(squeeze(matrix_train_60_40cs(trial,chn,:)),fc));
    end
end
...
for trial=1:rr
    for chn=1:32
        signal_STFT2(trial,chn,:,:)=abs(stft(squeeze(matrix_test_60_40cs(trial,chn,:)),fc));
    end
end
...
for trial=1:vv
    for chn=1:32
        k(trial,chn)=kurtosis(matrix_train_60_40cs(trial,chn,:));
    end
end
 % k=mean(k,2);
 ...
 for trial=1:rr
    for chn=1:32
        k2(trial,chn)=kurtosis(matrix_test_60_40cs(trial,chn,:));
    end
end
%  k2=mean(k2,2);
...
for trial = 1:vv
    for channel = 1:32
        signal = squeeze(matrix_train_60_40cs(trial, channel, :));
        % Calcolo la CWT
...
for trial =1:rr
    for channel = 1:32
        signal2= squeeze(matrix_test_60_40cs(trial, channel, :));
        % Calcolo la CWT
...
%skewnees
skew_data=skewness(matrix_train_60_40cs(1:vv,1:32,:),0,3);
skew_data2=skewness(matrix_test_60_40cs(1:rr,1:32,:),0,3);
...
for i = 1:size(filter_bands, 1)
    ...
    class1_data = matrix_train_60_40cs(labels_train_60_40cs(1:num_trials,1) == 1, :, :);
    class2_data = matrix_train_60_40cs(labels_train_60_40cs(1:num_trials,1) == 2, :, :);
...
for i = 1:size(filter_bands, 1)
    for trial = 1:num_trials2
        csp_projection = squeeze(matrix_train_60_40cs(trial, :, :))' * csp_filters{i};
...
for i = 1:size(filter_bands, 1)
    for trial=1:rr
        csp_projection = squeeze(matrix_test_60_40cs(trial, :, :))' * csp_filters{i};
...
eeg_pre=event-200;
eeg_post=event+300;
epochs=matrix_train_60_40cs(1:vv,:,eeg_pre:eeg_post-1);
...
epochs2=matrix_test_60_40cs(1:rr,:,eeg_pre:eeg_post-1);
...
XTrain(1:vv,:,:)=matrix_train_60_40cs(1:vv,:,:);
...
XTrain2(1:rr,:,:)=matrix_test_60_40cs(1:rr,:,:);
...
%coherence
features_chc=extractCoherenceFeatures(matrix_train_60_40cs,fc);
features_chc2=extractCoherenceFeatures(matrix_test_60_40cs,fc);
% for trial = 1:vv
%         for ch = 1:32
...
feature_sogg_60cs2_1=zeros(vv,18);
feature_sogg_60cs2_1(1:vv,1:32,1)=data1_m(1:vv,1:32);
feature_sogg_60cs2_1(1:vv,33:64,1)=data1_std(1:vv,1:32);
feature_sogg_60cs2_1(1:vv,65:96,1)=picco(1:vv,1:32);
feature_sogg_60cs2_1(1:vv,97:128,1)=rms_data(1:vv,1:32);
feature_sogg_60cs2_1(1:vv,129:160,1)=peaktopeak(1:vv,1:32);
feature_sogg_60cs2_1(1:vv,161:192,1)=signal_STFT(1:vv,1:32);
feature_sogg_60cs2_1(1:vv,193:224,1)=k(1:vv,1:32);
feature_sogg_60cs2_1(1:vv,225:288,1)=PSD(1:vv,1:64);
feature_sogg_60cs2_1(1:vv,289:320,1)=wavelet_energy(1:vv,1:32);
feature_sogg_60cs2_1(1:vv,321:352,1)=skew_data(1:vv,1:32);
feature_sogg_60cs2_1(1:vv,353:384,1)=data_var(1:vv,1:32);
feature_sogg_60cs2_1(1:vv,385:416,1)=erd_ers_per_trial(1:vv,1:32);
feature_sogg_60cs2_1(1:vv,417:434,1)=features(1:vv,1:18);
feature_sogg_60cs2_1(1:vv,435:466,1)=features_encode(1:vv,1:32);
feature_sogg_60cs2_1(1:vv,467:466+size(features_chc,2),1)=features_chc(1:vv,1:size(features_chc,2));
% feature_sogg_80pc(1:vv,962:993,1)=features_hig(1:vv,1:32);
% 
% 


feature_sogg_40cs2_1(1:rr,1:32,1)=data2_m(1:rr,1:32);
feature_sogg_40cs2_1(1:rr,33:64,1)=data2_std(1:rr,1:32);
feature_sogg_40cs2_1(1:rr,65:96,1)=picco2(1:rr,1:32);
feature_sogg_40cs2_1(1:rr,97:128,1)=rms_data2(1:rr,1:32);
feature_sogg_40cs2_1(1:rr,129:160,1)=peaktopeak2(1:rr,1:32);
feature_sogg_40cs2_1(1:rr,161:192,1)=signal_STFT2(1:rr,1:32);
feature_sogg_40cs2_1(1:rr,193:224,1)=k2(1:rr,1:32);
feature_sogg_40cs2_1(1:rr,225:288,1)=PSD2(1:rr,1:64);
feature_sogg_40cs2_1(1:rr,289:320,1)=wavelet_energy2(1:rr,1:32);
feature_sogg_40cs2_1(1:rr,321:352,1)=skew_data2(1:rr,1:32);
feature_sogg_40cs2_1(1:rr,353:384,1)=data_2var(1:rr,1:32);
feature_sogg_40cs2_1(1:rr,385:416,1)=erd_ers_per_trial2(1:rr,1:32);
feature_sogg_40cs2_1(1:rr,417:434,1)=features3(1:rr,1:18);
feature_sogg_40cs2_1(1:rr,435:466,1)=features_encode2(1:rr,1:32);
feature_sogg_40cs2_1(1:rr,467:466+size(features_chc2,2),1)=features_chc2(1:rr,1:size(features_chc2,2));
% feature_sogg_20pc(1:round(0.2*rr),962:993,1)=features_hig3(1:round(0.2*rr),1:32);

time_extraction_featurecs2_1=toc;
end